...
boolean inSummerTime(int uur, int dag, int maand, int jaar, int dw) {
  ...
  int dts = 8 - dw; // Number of days before the next sunday
  ...
  if ((maand >= 3) && (maand <= 10)) { // March to October inclusive
    ...
    if (maand == 3) { // Have we passed the last Sunday of March, 1am UT ?
      if (dag >= 25) { // It's getting interesting
        ...
        //int dts = 8 - dw; // Number of days before the next sunday
        if (dts == 7) dts = 8; // We are really speaking of the next sunday, not the current one
...
void SerialRTCtime(String input){
  ...
  //rtc.setTime(hr, m, s)  24h format
  //rtc.setDate(18, 4, 2020);   // Set the date to January 1st, 2014
...
void timeSync(){
  int s = second();
  timesyncCounter++;
  // voorkom dat sync plaatsvind rond wisseling van minuut.
  // i.v.m. schakelmomenten van diverse timers.
  if ((timesyncCounter == 300) && (timesync == false)){
    if ((s > 55) || (s < 5)){
      timesyncCounter = 290;
    }
    else {
      // sync arduino met RTC  
      setRTCtime();
      timesync = true;
    }
  }
  // geef timesync vrij na langere tijd om "pendelen" te voorkomen
